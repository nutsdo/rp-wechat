 ...
 @section('flash-message')
  ...
  <div class="col-md-9 mailbox-right">
    ...
    <div class="panel panel-default">
        ...
        {!! Form::open(['route'=>['ucenter.wechat.vote.update',$wechatId,$vote->id],'role'=>'form','class'=>'validate','method'=>'PATCH']) !!}

            <div class="form-group">
                <label class="control-label">名称</label>

                <input type="text" class="form-control" name="name" value="{{$vote->name}}" data-validate="required" data-message-required="This is custom message for required field." placeholder="名称" />
            </div>

            <div class="form-group @if($errors->first('cover_url')) has-error @endif">
                <label class="control-label" for="pic_url">封面</label><br>
                <div class="col-sm-10">
                    <input  type="text" name="pic_url" class="col-sm-11 form-control" value="{{$vote->pic_url}}" id="pic_url" placeholder="@if($errors->first('pic_url')) {{$errors->first('pic_url')}} @else 封面图片地址 @endif">
                </div>
                <div class="col-sm-1">
            ...
            <div class="form-group">
                ...
                <img id="preview" src="{{url($vote->pic_url)}}"/>
            </div>

            <div class="form-group">
                <label class="control-label">描述</label>

                <input type="text" class="form-control" name="description" value="{{$vote->description}}" data-validate="required" placeholder="描述" />
            </div>

            <div class="form-group">
                <label class="control-label">规则</label>

                <textarea type="text" class="form-control" name="rule_body" placeholder="规则" >{{$vote->rule_body}}</textarea>
            </div>

            <div class="form-group">
                <label class="control-label">每天投票次数</label>

                <input type="text" class="form-control" name="vote_times" value="{{$vote->vote_times}}" data-validate="required" placeholder="每天投票次数" />
            </div>
