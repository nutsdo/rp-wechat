    ...
    <div class="col-sm-12">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                {!! Form::open(['route'=>'admin.node.store','role'=>'form','class'=>'form-horizontal',]) !!}
                    ...
                    <div class="form-group @if($errors->first('node_parent_id')) has-error @endif">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" name="node_parent_id">
                                ...
                                <option value="{{$node->id}}" @if($id==$node->id) selected @endif>&nbsp;&equiv;{{$node->title}}</option>
                                @if(!empty($node->children))
                                    @foreach($node->children as $ck=>$cnode)
                                    <option value="{{$cnode->id}}" @if($id==$cnode->id) selected @endif>&nbsp;&nbsp;&nbsp;&equiv;{{$cnode->title}}</option>
                                    @if(!empty($cnode->children))
                                        @foreach($cnode->children as $sk=>$snode)
                                        <option value="{{$snode->id}}" @if($id==$snode->id) selected @endif>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&equiv;{{$snode->title}}</option>
                                        @endforeach
                                    @endif
                                    @endforeach
                                @endif
                    ...
                    <div class="form-group @if($errors->first('title')) has-error @endif">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="title" class="form-control" id="title" placeholder="@if($errors->first('title')) {{$errors->first('title')}} @else 节点名称 @endif">
                        </div>
                    </div>
                    ...
                    <div class="form-group @if($errors->first('description')) has-error @endif">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="description" class="form-control" id="description" placeholder="@if($errors->first('description')) {{$errors->first('description')}} @else 节点描述 @endif">
                        </div>
                    </div>
