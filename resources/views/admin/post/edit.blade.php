 				...
 				<div class="col-sm-12">
 					...
 					<div class="panel panel-default">
 						<div class="panel-heading">
 							<h3 class="panel-title">修改ID为<{{$post->id}}>内容信息</h3>
 						...
 						<div class="panel-body">
 							...
 							{!! Form::open(['route'=>['admin.post.update',$post->id],'role'=>'form','class'=>'form-horizontal','method'=>'patch']) !!}

                                <div class="form-group @if($errors->first('node_parent_id')) has-error @endif">
                                    <label class="col-sm-1 control-label" for="node_id">所属节点</label>
                                    ...
                                    <div class="col-sm-11">
                                        <select class="form-control" name="node_id">
                                            ...
                                            <option value="{{$node->id}}" @if($post->node_id==$node->id) selected @endif >&nbsp;&equiv;{{$node->title}}</option>
                                            @if(!empty($node->children))
                                                @foreach($node->children as $ck=>$cnode)
                                                <option value="{{$cnode->id}}" @if($post->node_id==$cnode->id) selected @endif>&nbsp;&nbsp;&nbsp;&equiv;{{$cnode->title}}</option>
                                                @if(!empty($cnode->children))
                                                    @foreach($cnode->children as $sk=>$snode)
                                                    <option value="{{$snode->id}}" @if($post->node_id==$snode->id) selected @endif>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&equiv;{{$snode->title}}</option>
                                                    @endforeach
                                                @endif
                                                @endforeach
                                            @endif
                                ...
                                <div class="form-group-separator"></div>
 								...
 								<div class="form-group @if($errors->first('title')) has-error @endif">
 									...
 									<div class="col-sm-11">
 										<input type="text" name="title" class="form-control" id="title" value="{{$post->title}}" placeholder="@if($errors->first('title')) {{$errors->first('title')}} @else 标题 @endif">
 									</div>
 								</div>
 								...
 								<div class="form-group @if($errors->first('photo')) has-error @endif">
 									...
 									<div class="col-sm-10">
 										<input type="text" name="photo" class="form-control"  id="photo" value="{{$post->photo}}" placeholder="@if($errors->first('photo')) {{$errors->first('photo')}} @else 封面图片地址 @endif">
 									</div>
                                ...
                                <div class="form-group @if($errors->first('post_url')) has-error @endif">
                                    <label class="col-sm-2 control-label" for="post_url">文章连接</label>

                                    <div class="col-sm-10">
                                        <input type="text" name="post_url" class="form-control" id="post_url" value="{{$post->post_url}}" placeholder="@if($errors->first('post_url')) {{$errors->first('post_url')}} @else 文章连接 @endif">
                                    </div>
                                </div>

                                <div class="form-group-separator"></div>

                                <div class="form-group @if($errors->first('shop_name')) has-error @endif">
                                    <label class="col-sm-2 control-label" for="shop_name">店铺名称</label>

                                    <div class="col-sm-10">
                                        <input type="text" name="shop_name" class="form-control" id="shop_name" value="{{$post->shop_name}}" placeholder="@if($errors->first('shop_name')) {{$errors->first('shop_name')}} @else 店铺名称 @endif">
                                    </div>
                                </div>
