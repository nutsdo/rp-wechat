            ...
            <div class="panel-body">
                ...
                {!! Form::open(['route'=>'admin.post.store','role'=>'form','class'=>'form-horizontal',]) !!}
                    ...
                    <div class="form-group @if($errors->first('node_parent_id')) has-error @endif">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" name="node_id">
                                ...
                                <option value="{{$node->id}}">&nbsp;&equiv;{{$node->title}}</option>
                                @if(!empty($node->children))
                                    @foreach($node->children as $ck=>$cnode)
                                    <option value="{{$cnode->id}}">&nbsp;&nbsp;&nbsp;&equiv;{{$cnode->title}}</option>
                                    @if(!empty($cnode->children))
                                        @foreach($cnode->children as $sk=>$snode)
                                        <option value="{{$snode->id}}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&equiv;{{$snode->title}}</option>
                                        @endforeach
                                    @endif
                                    @endforeach
                                @endif
                    ...
                    <div class="form-group @if($errors->first('photo')) has-error @endif">
                        <label class="col-sm-2 control-label" for="photo">封面</label>
                    ...
                    <div class="form-group @if($errors->first('post_url')) has-error @endif">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="post_url" class="form-control" id="post_url" placeholder="@if($errors->first('post_url')) {{$errors->first('post_url')}} @else 文章连接 @endif">
                        </div>
                    </div>
                    ...
                    <div class="form-group @if($errors->first('shop_name')) has-error @endif">
                        <label class="col-sm-2 control-label" for="post_url">店铺名称</label>

                        <div class="col-sm-10">
                            <input type="text" name="shop_name" class="form-control" id="shop_name" placeholder="@if($errors->first('shop_name')) {{$errors->first('shop_name')}} @else 店铺名称 @endif">
                        </div>
                    </div>
